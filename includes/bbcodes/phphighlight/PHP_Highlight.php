...
class PHP_Highlight {
    ...
    function PHP_Highlight($inline = FALSE) {
        ...
        if ($inline === FALSE) {
            ...
            $this->highlight = ['string'  => ini_get('highlight.string'), 'comment' => ini_get('highlight.comment'),
            ...
            // Basic styles
            $this->highlight = ['string'  => 'string', 'comment' => 'comment', 'keyword' => 'keyword', 'bg' => 'bg',
    ...
    function toList($return = FALSE, $funcref = TRUE, $blocks = TRUE) {
        ...
        foreach ($source as $i => $line) {
            ...
            if (empty($line)) {
                $out .= '&nbsp;';
            } else if ($blocks === TRUE && in_array($i, $this->_plaintextkeys)) {
                $out .= $line;
            } else {
                $out .= "<code>$line</code>";
            }
    ...
    function toHtmlBlocks($return = FALSE, $linenum = FALSE, $format = NULL, $reset = TRUE, $funcref = TRUE) {
        ...
        foreach ($source as $i => $line) {
            ...
            if (empty($line)) {
                ...
                // Plain text
            } else if (in_array($i, $this->_plaintextkeys)) {
                if ($wasplain === FALSE) {
                    $out .= "</code>\n";
                    // Reset line numbers
                    if ($reset === TRUE) {
