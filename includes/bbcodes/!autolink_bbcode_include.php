...
if ($codde = substr_count($text, "[code]") > 0)
    $text = preg_replace("#\[code\](.*?)\[/code\]#sie", "'[code]'.bbcode_off('\\1', '1').'[/code]'", $text);
if ($geshii = substr_count($text, "[geshi=") > 0)
    $text = preg_replace("#\[geshi=(.*?)\](.*?)\[/geshi\]#sie", "'[geshi=\\1]'.bbcode_off('\\2', '1').'[/geshi]'", $text);
if ($phpp = substr_count($text, "[php]") > 0)
    $text = preg_replace("#\[code\](.*?)\[/code\]#sie", "'[code]'.bbcode_off('\\1', '1').'[/code]'", $text);
$text = str_replace(["]", "&gt;", "[", "&lt;"], ["]&nbsp;", "&gt; ", " &nbsp;[", " &lt;"], $text);
...
if ($codde > 0)
    $text = preg_replace("#\[code\](.*?)\[/code\]#sie", "'[code]'.bbcode_off('\\1', '2').'[/code]'", $text);
if ($geshii > 0)
    $text = preg_replace("#\[geshi=(.*?)\](.*?)\[/geshi\]#sie", "'[geshi=\\1]'.bbcode_off('\\2', '2').'[/geshi]'", $text);
if ($phpp > 0)
    $text = preg_replace("#\[php\](.*?)\[/php\]#sie", "'[php]'.bbcode_off('\\1', '2').'[/php]'", $text);
$text = str_replace(["]&nbsp;", "&gt; ", " &nbsp;[", " &lt;"], ["]", "&gt;", "[", "&lt;"], $text);
