...
class UserFieldsInput {
    ...
    public function saveInsert() {
        ...
        $this->userData = ["user_password" => "", "user_algo" => "", "user_salt" => "", "user_admin_password" => "", "user_admin_algo" => "", "user_admin_salt" => "", "user_name" => "", "user_email" => ""];
        $this->_fieldsRequired = ["user_name" => TRUE, "user_password" => TRUE, "user_email" => TRUE, "user_captcha" => TRUE, "email_activation" => TRUE];
    ...
    public function saveUpdate() {
        ...
        if (isset($_GET['aid'])) {
            ...
            if (!isset($_GET['profiles'])) {
                ...
                $this->_setEmptyFields();
            } else if (isset($_GET['profiles']) && ($_GET['profiles'] == 'avatar')) {
                $this->_setUserAvatar();
            } else {
                $this->_setCustomUserFieldsData();
            }
    ...
    private function _setEmailVerification() {
        ...
        if (sendemail($this->_userName, $this->_userEmail, $settings['siteusername'], $settings['siteemail'], $locale['u151'], $message)) {
            $userInfo = serialize(["user_name"         => $this->_userName, "user_password" => $this->_newUserPasswordHash,
    ...
    private function _setUserDataInput() {
        ...
        if ($this->adminActivation) {
            ...
            if (!$this->isAdminPanel) {
                ...
                $replace_this = ["[USER_NAME]", "[PASSWORD]"];
                $replace_with = [$this->_userName, $this->_newUserPassword];
