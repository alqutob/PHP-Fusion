...
if (isset($_POST['postreply']) && !defined('FUSION_NULL')) {
    ...
    echo openform('input_form', 'input_form', 'post', "".($settings['site_seo'] ? FUSION_ROOT : '').FORUM."post.php?action=reply&amp;forum_id=".$_GET['forum_id']."&amp;thread_id=".$_GET['thread_id'], ['enc_type' => 1]);
    echo "<table cellpadding='0' cellspacing='1' width='100%' class='tbl-border table table-responsive'>\n<tbody>\n<tr>\n";
    echo "<td valign='top' width='145' class='tbl2'><label for='message'>".$locale['461']."</label><span class='required'>*</span></td>\n";
    echo "<td class='tbl1'>\n";
    echo form_textarea('', 'message', 'message', $message, ['bbcode' => 1, 'required' => 1]);
    echo "</td>\n</tr>\n";
    ...
    echo $settings['site_seo'] ? '' : form_button($locale['402'], 'previewreply', 'previewreply', $locale['402'], ['class' => 'btn-primary m-r-10']);
    echo form_button($locale['404'], 'postreply', 'postreply', $locale['404'], ['class' => 'btn-primary m-r-10']);
    echo "</td>\n</tr>\n</tbody>\n</table>\n";
    echo closeform();
    closetable();
    echo "<!--sub_postreply-->";
