...
if (isset($_POST['savechanges'])) {
    ...
    echo openform('input_form', 'input_form', 'post', FUSION_SELF."?action=edit&amp;forum_id=".$_GET['forum_id']."&amp;thread_id=".$_GET['thread_id']."&amp;post_id=".$_GET['post_id'], ['enc_type' => 1]);
    ...
    if ($pdata['first_post'] == $_GET['post_id']) {
        ...
        echo "<td class='tbl2'>\n";
        echo form_text('', 'subject', 'subject', $subject, ['max_length' => 255, 'required' => 1]);
        echo "</td>\n</tr>\n<tr>\n";
    }
    echo "<td valign='top' width='145' class='tbl2'><label for='message'>".$locale['461']."</label> <span class='required'>*</span></td>\n";
    echo "<td class='tbl1'>\n";
    echo form_textarea('', 'message', 'message', $message, ['requried' => 1, 'bbcode' => 1]);
    echo "</td>\n</tr>\n";
    ...
    if ($fdata['forum_poll'] && checkgroup($fdata['forum_poll'])) {
        if ($tdata['thread_poll'] && ($pdata['post_author'] == $tdata['thread_author']) && ($userdata['user_id'] == $tdata['thread_author'] || iSUPERADMIN || iMOD)) {
            ...
            echo "<td class='tbl1'>\n";
            echo form_text('', 'poll_title', 'poll_title', $poll_title, ['max_length' => 255]);
            echo form_button($locale['472'], 'update_poll_title', 'update_poll_title', $locale['472'], ['class' => 'btn-default m-r-10']);
            ...
            if (isset($poll_opts) && is_array($poll_opts)) {
                foreach ($poll_opts as $poll_option) {
                    ...
                    echo "<td class='tbl1'>\n";
                    echo form_text('', "poll_options[$i]", "poll_options[$i]", $poll_option, ['max_length' => 255]);

                    echo form_button($locale['472'], "update_poll_option[$i]", "update_poll_option[$i]", $locale['472'], ['class' => 'btn-default m-r-10']);
                    echo form_button($locale['473'], "delete_poll_option[$i]", "delete_poll_option[$i]", $locale['473'], ['class' => 'btn-default m-r-10']);
                    echo "</td>\n</tr>\n";
                    $i++;
                }
                echo "<tr>\n<td width='145' class='tbl2'><label for='poll_options[$i]'>".$locale['470']." ".$i."</label></td>\n";
                echo "<td class='tbl1'>\n";
                echo form_text('', "poll_options[$i]", "poll_options[$i]", '', ['max_length' => 255]);
    ...
    echo form_button($locale['405'], "previewchanges", "previewchanges", $locale['405'], ['class' => 'btn-primary m-r-10']);
    echo form_button($locale['409'], "savechanges", "savechanges", $locale['409'], ['class' => 'btn-primary m-r-10']);
    echo "</td>\n</tr>\n</table>\n</form>\n";
    closetable();
}
