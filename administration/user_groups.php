...
if (isset($_POST['save_group'])) {
    ...
    if ($check_count > 0) {
        ...
        while ($data = dbarray($result)) {
            $user_groups = preg_replace(["(^\.{$_GET['group_id']}$)", "(\.{$_GET['group_id']}\.)",
                                         "(\.{$_GET['group_id']}$)"], ["", ".", ""], $data['user_groups']);
            $result2 = dbquery("UPDATE ".DB_USERS." SET user_groups='$user_groups' WHERE user_id='".$data['user_id']."'");
        }
...
if (dbrows($result)) {
    ...
    echo openform('selectform', 'selectform', 'post', FUSION_SELF.$aidlink, ['downtime' => 0, 'notice' => 0]);
    $sel_opts = [];
    while ($data = dbarray($result)) {
        $sel_opts[$data['group_id']] = "ID: ".$data['group_id']." - ".$data['group_name'];
    }
    echo form_select('', 'group_id', 'group_id', $sel_opts, '', ['placeholder' => $locale['choose'],
                                                                 'class'       => 'pull-left']);
    echo form_button($locale['421'], 'edit', 'edit', $locale['421'], ['class' => 'btn-primary m-l-10 pull-left']);
    echo form_button($locale['422'], 'delete', 'delete', $locale['422'], ['class' => 'btn-primary m-l-10 pull-left']);
    echo closeform();
    closetable();
}
if (isset($_GET['group_id']) && isnum($_GET['group_id'])) {
...
echo openform('editform', 'editform', 'post', $form_action, ['downtime' => 0]);
echo "<table cellpadding='0' cellspacing='0' class='table table-responsive center'>\n<tbody>\n";
echo "<tr>\n<td class='tbl' width='1%' style='white-space:nowrap;'><label for='group_name'>".$locale['432']."</label></td>\n";
echo "<td class='tbl'>\n";
echo form_text('', 'group_name', 'group_name', $group_name, ['required' => 1, 'error_text' => $locale['464']]);
...
echo form_button($locale['434'], 'save_group', 'save_group', $locale['434'], ['class' => 'btn-primary']);
echo "</td>\n</tr>\n</tbody>\n</table>\n</form>";
closetable();
if (isset($_GET['group_id']) && isnum($_GET['group_id'])) {
    ...
    if (!isset($_POST['search_users'])) {
        ...
        echo "</tr>\n<tr>\n<td align='center' class='tbl'>\n";
        echo form_button($locale['445'], 'search_users', 'search_users', $locale['445'], ['class' => 'btn-primary']);
        echo "</td>\n</tr>\n</table>\n</form>\n";
    }
    if (isset($_POST['search_users']) && isset($_POST['search_criteria'])) {
        ...
        if (isset($result) && dbrows($result)) {
            ...
            if ($i > 0) {
                ...
                echo form_button($locale['450'], 'add_sel', 'add_sel', $locale['450'], ['class' => 'btn-primary']);
                echo "</td>\n</tr>\n";
            } else {
    ...
    if ($rows) {
        ...
        echo form_button($locale['461'], 'remove_sel', 'remove_sel', $locale['461'], ['class' => 'btn-primary m-r-10']);
        echo form_button($locale['462'], 'remove_all', 'remove_all', $locale['462'], ['class' => 'btn-primary']);
        echo "</td>\n</tr>\n";
    } else {
