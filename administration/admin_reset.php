    ...
    if (check_admin_pass(isset($_POST['admin_password']) ? stripinput($_POST['admin_password']) : "")) {
        ...
        while ($data = dbarray($result)) {
            ...
            if (isset($_POST['reset_login']) && $_POST['reset_login'] == 1) {
                ...
                $message = str_replace(["[USER_NAME]", "[NEW_PASS]", "[NEW_ADMIN_PASS]", "[ADMIN]",
                                        "[RESET_MESSAGE]"], [$data['user_name'], $newLoginPass, $newAdminPass,
                                                             $userdata['user_name'],
                                                             $reset_message], $locale['409']);
                $loginPassIsReset = ($loginPass->isValidNewPassword() === 0 ? TRUE : FALSE);
            } else {
                $message = str_replace(["[USER_NAME]", "[NEW_ADMIN_PASS]", "[ADMIN]",
                                        "[RESET_MESSAGE]"], [$data['user_name'], $newAdminPass,
                                                             $userdata['user_name'],
...
$reset_opts = ['all' => $locale['401'], 'sa' => $locale['402'], 'a' => $locale['403']];
...
echo "<td class='tbl1'>".form_select('', 'reset_admin', 'reset_admin', $reset_opts, '', ['placeholder' => $locale['choose'],
                                                                                         'allowclear'  => 1]);
echo "</td>\n";
echo "</tr>\n<tr>\n";
...
echo "</tr>\n<tr>\n";
echo "<td class='tbl1' width='250' valign='top'></td>\n";
echo "<td class='tbl1'>".form_button($locale['406'], 'reset_admins', 'reset_admins', $locale['406'], ['class' => 'button btn btn-primary'])."</td>\n";
echo "</tr>\n</table>\n</form>\n";
closetable();
