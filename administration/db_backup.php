...
if (!checkrights("DB") || !defined("iAUTH") || !isset($_GET['aid']) || $_GET['aid'] != iAUTH)
    redirect("../index.php");
// Unstrip text
...
if (isset($_POST['btn_do_restore'])) {
    ...
    if ((preg_match("/# Database Name: `(.+?)`/i", $result[2], $tmp1)) && (preg_match("/# Table Prefix: `(.+?)`/i", $result[3], $tmp2)) && !defined('FUSION_NULL')) {
            ...
            foreach ($results as $result) {
                ...
                if (preg_match("/^DROP TABLE IF EXISTS `(.*?)`/im", $result, $tmp)) {
                    ...
                    if (in_array($tbl, $_POST['list_tbl'])) {
                        ...
                        if ($pdo_enabled == "1") {
                            $rct1 = $pdo->prepare($result, [PDO::MYSQL_ATTR_USE_BUFFERED_QUERY => TRUE]);
                            $rct1->execute();
                        } else {
                ...
                if (preg_match("/^CREATE TABLE `(.*?)`/im", $result, $tmp)) {
                    ...
                    if (in_array($tbl, $_POST['list_tbl'])) {
                        ...
                        if ($pdo_enabled == "1") {
                            $rct2 = $pdo->prepare($result, [PDO::MYSQL_ATTR_USE_BUFFERED_QUERY => TRUE]);
                            $rct2->execute();
                        } else {
                            mysql_unbuffered_query($result);
                        }
            ...
            foreach ($results as $result) {
                if (preg_match("/INSERT INTO `(.*?)`/i", $result, $tmp)) {
                    ...
                    if (in_array($ins, $_POST['list_ins'])) {
                        ...
                        if ($pdo_enabled == "1") {
                            $rct3 = $pdo->prepare($result, [PDO::MYSQL_ATTR_USE_BUFFERED_QUERY => TRUE]);
                            $rct3->execute();
                        } else {
                            mysql_unbuffered_query($result);
                        }
        ...
        echo form_button($locale['403'], 'btn_cancel', 'btn_cancel', $locale['403'], ['class' => 'btn-primary']);
        echo "</form>\n</div>\n";
        closetable();
    }
    ...
    echo form_text($locale['415'], 'restore_tblpre', 'restore_tblpre', $info_tblpref, ['required'   => 1,
                                                                                       'error_text' => '']);
    echo "</td>\n</tr>\n<tr>\n";
    ...
    echo "</tr>\n<tr>\n";
    echo "<td colspan='2' class='tbl'>\n";
    echo form_text($locale['460'], 'user_admin_password', 'user_admin_password', '', ['required'   => 1,
                                                                                      'error_text' => $locale['460b'],
                                                                                      'inline'     => 1]);
    echo "</tr>\n<tr>\n";
    echo "<td align='center' colspan='2' class='tbl'>\n";
    echo "<input type='hidden' name='file' value='$file' />\n";
    echo form_button($locale['438'], 'btn_do_restore', 'btn_do_restore', $locale['438'], ['class' => 'btn-primary m-r-10']);
    echo form_button($locale['439'], 'btn_cancel', 'btn_cancel', $locale['439'], ['class' => 'btn-primary m-r-10']);
    echo "</tr>\n</tbody>\n</table>\n";
    echo closeform();
    closetable();
} else {
    ...
    echo "//-->\n</script>\n";
    echo openform('backupform', 'backupform', 'post', FUSION_SELF.$aidlink, ['downtime' => 0]);
    echo "<table align='center' cellspacing='0' cellpadding='0' class='table table-responsive'>\n<tbody>\n<tr>\n";
    echo "<td valign='top'>\n";
    ...
    echo "<td class='tbl'>\n";
    echo form_text('', 'backup_filename', 'backup_filename', "backup_".stripsiteinput($settings['sitename'])."_".date('Y-m-d-Hi')."", ['required'   => 1,
    ...
    $opts['.sql'] = ".sql";
    echo form_select('', 'backup_type', 'backup_type', $opts, '', ['placeholder' => $locale['choose']]);
    echo "</td>\n</tr>\n<tr>\n";
    echo "<td align='right' class='tbl'><label for='user_admin_password'>".$locale['460']."</label> <span class='required'>*</span></td>\n";
    echo "<td class='tbl'>\n";
    echo form_text('', 'user_admin_password', 'user_admin_password', '', ['password'   => 1, 'required' => 1,
                                                                          'error_text' => $locale['460b']]);
    echo "</td>\n</tr>\n<tr>\n";
    ...
    echo "</tr>\n<tr>\n";
    echo "<td align='center' colspan='2' class='tbl'>";
    echo form_button($locale['459'], 'btn_create_backup', 'btn_create_backup', $locale['459'], ['class' => 'btn-primary']);
    echo "</td>\n</tr>\n</tbody>\n</table>\n</form>\n";
    closetable();
    opentable($locale['480']);
    $file_types = (function_exists("gzencode")) ? ".gz " : ""; // added
    echo openform('restore', 'restore', 'post', FUSION_SELF.$aidlink."&amp;action=restore", ['enctype' => 1]);
    ...
    echo form_button($locale['438'], 'restore', 'restore', $locale['438'], ['class' => 'btn-primary']);
    echo "</td>\n</tr>\n</tbody>\n</table>\n";
    echo closeform();
    closetable();
}
