...
if (function_exists('gd_info')) {
    ...
    echo openform('input_form', 'input_form', 'post', $formaction, ['enctype' => 1, 'downtime' => 0]);
    if (isset($_GET['action']) && $_GET['action'] == "edit") {
        ...
        if (dbrows($result2)) {
            ...
            echo form_button($locale['431'], 'move_photo', 'move_photo', $locale['431'], ['class' => 'btn-primary']);
            echo form_select('', 'move_album_id', 'move_album_id', $array, '', ['placeholder' => $locale['473'], 'class' => 'pull-left m-r-10']);
            echo "</div></div>\n";
        }
    }
    ...
    echo form_text($locale['432'], 'photo_title', 'photo_title', $photo_title, ['max_length' => 100, 'required' => 1, 'error_text' => '']);
    echo form_textarea($locale['433'], 'photo_description', 'photo_description', $photo_description, ['bbcode' => 1]);
    if (!isset($_GET['action'])) {
        echo form_fileinput($locale['436'], 'photo_pic_file', 'photo_pic_file', PHOTODIR, '', ['image' => 1, 'required' => 1, 'error_text' => $locale['421']]);
    ...
    echo form_text($locale['434'], 'photo_order', 'photo_order', $photo_order, ['number' => 1, 'width' => '100px']);
    ...
    echo form_button($locale['439'], 'save_photo', 'save_photo', $locale['439'], ['class' => 'btn-primary']);
    if (isset($_GET['action']) && $_GET['action'] == "edit") {
        echo form_button($locale['440'], 'cancel', 'cancel', $locale['440'], ['class' => 'btn-primary m-l-10']);
    }
    echo "</div>";
    echo "</div>\n";
    echo closeform();
    ...
    if (!isset($_GET['action'])) {
        ...
        if ($can_upload == TRUE) {
            ...
            echo openform('folderuploadform', 'folderuploadform', 'post', FUSION_SELF.$aidlink."&amp;album_id=".$_GET['album_id'], ['enctype' => 1, 'downtime' => 0]);
            ...
            if ($folder_opts != "") {
                ...
                echo form_button($locale['500'], 'btn_upload_dir', 'btn_upload_dir', $locale['500'], ['class' => 'btn-primary btn-block']);
                ...
                echo form_button($locale['504'], 'refresh', 'refresh', $locale['504'], ['class' => 'btn-primary btn-block']);
            ...
            echo openform('why_no_formname', 'why_no_formname', 'post', $formaction, ['downtime' => 0]);
            echo "<span style='color:red;font-weight:bold;'>".sprintf($locale['502'], $upload_dir)."</span><br />".$locale['503']."<br />";
            echo form_button($locale['504'], 'refresh2', 'refresh2', $locale['504'], ['class' => 'btn-primary btn-block']);
        ...
        if ($can_upload == TRUE) {
            ...
            echo openform('multiform', 'multiform', 'post', FUSION_SELF.$aidlink."&amp;album_id=".$_GET['album_id'], ['downtime' => 0, 'enctype' => 1]);
            echo "<table class='table table-responsive tbl-border center' cellpadding='2' cellspacing='0'>\n";
            echo($multi_opts != "" ? "<tr>\n<td class='tbl1' colspan='2' style='text-align:center;'>".$locale['496']."</td>\n</tr>\n" : "");
            echo "<tr>\n<td class='tbl1' colspan='2' style='text-align:left;'>";
            ...
            if ($multi_opts != "") {
                ...
                echo form_button($locale['509'], 'btn_multi_upload', 'btn_multi_upload', $locale['509'], ['class' => 'btn-primary btn-block']);
                echo "</td>\n</tr>\n";
            } else {
                echo "<tr>\n<td class='tbl1' colspan='2' style='text-align:center;'>".$locale['510']."<br /><br />\n";
                //echo "<input type='submit' class='button' value='".$locale['504']."' /></td>\n</tr>\n";
                echo form_button($locale['504'], 'btn_multi_noname', 'btn_multi_noname', $locale['504'], ['class' => 'btn-primary btn-block']);
            }
            echo "</table>";
            echo "</form>\n";
        } else {
            echo "<div class='admin-message'>\n";
            //echo "<form action='".$formaction."' method='post'>\n";
            echo openform('multi', 'multi', 'post', $formaction, ['downtime' => 0]);
            echo "<span style='color:red;font-weight:bold;'>".sprintf($locale['502'], $upload_dir)."</span><br />".$locale['503']."<br />";
            echo "<input type='submit' class='button' value='".$locale['504']."' />";
            echo form_button($locale['509'], 'btn_multi_upload', 'btn_multi_upload', $locale['509'], ['class' => 'btn-primary btn-block']);
            echo "</form>\n";
    ...
    if ($rows) {
        ...
        echo openform('move_form', 'move_form', 'post', FUSION_SELF.$aidlink."&amp;album_id=".$_GET['album_id'], ['notice' => 0, 'downtime' => 0]);
        echo "<div class='row'>\n";
        ...
        if (dbrows($result) > 0) {
            ...
            echo form_button($locale['474'], 'move_sel_photos', 'move_sel_photos', $locale['474'], ['class' => 'btn-default']);
            echo form_button($locale['475'], 'move_all_photos', 'move_all_photos', $locale['475'], ['class' => 'btn-default']);
            add_to_jquery("
            ...
            echo form_select('', 'move_album_id', 'move_album_id2', $opts, '', ['placeholder' => $locale['473'], 'allowclear' => 1, 'class' => 'display-block m-b-10']);
        }
