...
if (!check_admin_pass($admin_password) && !stristr($_SERVER['PHP_SELF'], $settings['site_path']."infusions")) {
    echo openform('admin-login-form', 'admin-login-form', 'post', $form_action, ['downtime' => 0]);
    openside('');
    ...
    echo form_text('', 'admin_password', 'admin_password', '', ['placeholder' => $locale['281'], 'autocomplete_off' => 1, 'password' => 1, 'required' => 1]);
    echo "</div>\n";
    echo "</div>\n";
    echo "</div>\n";
    closeside();
    echo form_button('Sign in', 'admin_login', 'admin_login', 'Sign in', ['class' => 'btn-primary btn-block']);
    echo closeform();
} else {
