...
if ($_GET['page'] == 1) {
    ...
    if (count($available_bbcodes) != $enabled) {
        ...
        for ($lp = 0; $lp < count($available_bbcodes); $lp++) {
            ...
            if (!in_array($available_bbcodes[$lp], $enabled_bbcodes)) {
                ...
                } else if (file_exists(LOCALE."English/bbcodes/".$available_bbcodes[$lp].".php")) {
    ...
    echo $navigation;
    echo openform('input_form', 'input_form', 'post', FUSION_SELF.$aidlink."&amp;page=2", ['downtime' => 0]);
    echo "<table cellspacing='0' cellpadding='0' class='table table-responsive center'>\n<tr>\n";
    echo "<td class='tbl'>\n";
    echo form_textarea($locale['418a'], 'test_message', 'test_message', $test_message, ['required'   => 1,
    ...
    echo form_button($locale['401'], 'post_test', 'post_test', $locale['401'], ['class' => 'btn-primary']);
    echo "</td>\n</tr>\n</table>\n</form>\n";
    closetable();
}
